...
management_data_cal <- 
        function(dt,time_span,period_type,tag){
                        ...
                        function(labor.table){
                                ...
                                labor.all <- 
                                        ...
                                        add_row(
                                                ...
                                                value=c(labor.table$产品,labor.table$分子,labor.table$细胞,
                        ...
                        fee.all <- 
                                map_dfr(names(seals.ratio.not_others),function(type){
                                        tibble::tibble(
                                                ...
                                                value=c(
                                                        daily.operation.fee$生产*contribute_ratio[type],
                                                        daily.operation.fee$产品*contribute_ratio[type],
                                                        daily.operation.fee$销售*seals.ratio.not_others[type],
                                                        daily.operation.fee$企管*seals.ratio.not_others[type],
                ...
                ##材料出库，测序，包装成本
                cost.export <- dplyr::filter(supp.dt[["材料、基因合成、测序等"]],
                                             类型=='材料') %>% 
                        select(-类型)
                cost.seq <- dplyr::filter(supp.dt[["材料、基因合成、测序等"]],
                                          类型=='基因合成+测序成本')%>% 
                        select(-类型)
                cost.pack <- dplyr::filter(supp.dt[["材料、基因合成、测序等"]],
                                           类型=='干冰泡沫箱')%>% 
                        select(-类型)
